...
class MeldingRepositoryPostgres(private val ds: DataSource) : MeldingRepository {
    override fun hentMeldingstyper() =
    ...
    override fun hentMeldingstype(meldingstype: String) =
        using(sessionOf(ds)) { session ->
            session.run(
                queryOf(
                    //language=PostgreSQL
    ...
    override fun hentMeldinger(meldingstype: String) =
        using(sessionOf(ds)) { session ->
            session.run(
                queryOf(
                    //language=PostgreSQL
    ...
    override fun lagre(melding: Meldingsinformasjon) {
        using(sessionOf(ds)) { session ->
    ...
    private fun Session.lagreMeldingstype(
        navn: String,
        type: String,
    ) {
        run(
    ...
    private fun Session.tellMelding(
        uuid: UUID,
        navn: String,
    ...
    private fun Session.lagreBehandlingskjede(
